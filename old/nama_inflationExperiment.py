...
def generateMinibatches(trainingDF,cuda=False,max_len=100,base_size=10,shuffle=True):
...
def trainMinibatch(batchData,modelPackage,inflation=0):
    ...
    d = F.pairwise_distance(vecs[0],vecs[1])

    # BCE loss if p(d) = e^-d
    loss = (match*d).sum()
    loss += -((1-match)*torch.log(1 - torch.exp(-d))).sum()

    if inflation:
        # Inflation essentially adds a dummy vector in the middle of the batch vectors
        # and trains the model as if it is an incorrect match for all of them
        # (simpler than generating negative matches through pairwise resampling)

        with torch.no_grad():
            vec_mean = sorted_vecs.mean(dim=0)

        d_mean = F.pairwise_distance(sorted_vecs,vec_mean)

        # BCE loss for the case where p(d) = e^-d and all targets are zero:
        loss += -inflation*torch.log(1 - torch.exp(-d_mean)).sum()
        #
        # loss += -inflation*torch.log(1 - torch.exp(-(sorted_vecs**2).sum(dim=1))).sum()
...
def trainModel(modelPackage,trainingDF,minibatch_size=10,inflation=0.01,epochs=10,exit_function=None,verbose=False):
    cuda = next(modelPackage['model'].parameters()).is_cuda
    epochBatchIterator = (generateMinibatches(trainingDF,cuda=cuda,base_size=minibatch_size) for epoch in range(epochs))
    bar_freq = max(1,int((len(trainingDF)/minibatch_size)/100))
    modelPackage['loss_history'] = trainWithHistory(lambda b: trainMinibatch(b,modelPackage,inflation=inflation),epochBatchIterator,modelPackage['loss_history'],
...
def findMatches(queryStrings,candidateStrings,modelPackage,n_matches=1):
    ...
    resultsDF['score'] = np.exp(-pairDistances)
...
modelPackage = newModel(cuda=True,d=500,lr=0.0001,weight_decay=1e-8,recurrent_layers=1,bidirectional=True)

#Repeat as desired:
for i in range(30,50):
    inflation = float(10*np.exp(-0.5*i))
    historyDF = trainModel(modelPackage,trainingDF,epochs=1,minibatch_size=20,inflation=inflation,verbose=True)
plotLossHistory(historyDF,ylim=(0,3))
...
sampleDF[sampleDF['query_string'].isin(matchesDF[matchesDF['match']==0]['query_string'])]

#
#
packed,selector,match = next(generateMinibatches(trainingDF,cuda=True,base_size=20))
#
#
# # W = packed
