...
def trainModel(modelPackage,trainingDF,minibatch_size=10,resample_prob=0.5,epochs=10,lr=(0.001,0.0001),exit_function=None,verbose=True):
    ...
    for i in range(epochs):
        ...
        epochBatchIterator = (generateMinibatches(trainingDF,cuda=cuda,base_size=minibatch_size,resample_prob=resample_prob),)
...
if __name__ == '__main__':
    ...
    # trainingDF = trainingDF.sample(100)

    modelPackage = newModel(cuda=True,d=500,weight_decay=1e-8,recurrent_layers=1,bidirectional=True)

    #Repeat as desired:
    historyDF = trainModel(modelPackage,trainingDF,epochs=20,minibatch_size=20,resample_prob=0.1,lr=(0.001,0.0001))
    plotLossHistory(historyDF)

    saveModelPackage(modelPackage, os.path.join(modelDir,'allTrainingData_bi_d500.bin'))
