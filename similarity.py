...
def loadModelPackage(filename,cuda=False):
...
def trainModel(modelPackage,trainingDF,testDF=None,save_as=None,hash_function=None,minibatch_size=10,resample_ratio=5,epochs=10,lr=0.001,exit_function=None,verbose=True):
    ...
    incorrectPairs = {(s0,s1) for i,s0,s1,m in trainingDF[['query_string','candidate_string','match']].itertuples() if m < 0.5}
...
def findFuzzyMatches(strings,modelPackage,min_score=0.95,batch_size=100,drop_duplicates=True):
...
#     '''
#     strings = sorted(set(query_strings) | set(candidate_strings))
#
#     correctPairs = set()
#     incorrectPairs = set()
#
...
#
#     componentMap = connectedStrings(strings,correct_pairs=correctPairs,incorrect_pairs=incorrectPairs,hash_function=hash_function)
#
#     queryDF = pd.DataFrame(sorted(set(query_strings)),columns=['query_string'])
#     candidateDF = pd.DataFrame(sorted(set(query_strings)),columns=['candidate_string'])
#
#     queryDF['component'] = queryDF['query_string'].apply(lambda s: componentMap[s])
#     candidateDF['component'] = candidateDF['candidate_string'].apply(lambda s: componentMap[s])
#
#     matchesDF = pd.merge(queryDF,candidateDF)
#
#     return matchesDF
#
#
...
# corpHash('Quest Diagnostics Incorporated')==corpHash('Quest Diagnostics')
...
#                             save_as=os.path.join(modelDir,'allTrainingData_3bi200_to_300.004.bin'))
...
#
#     testResults = scoreTestDF(modelPackage,testDF.dropna(),corpHash)
#
