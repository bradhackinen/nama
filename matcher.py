...
class Matcher():
    ...
    def edge_compare(self, matcher):

        n = len(self.strings(min_count=0))

        edges_A = {tuple(sorted((i, j))) for i, j in self.G.edges()}
        edges_B = {tuple(sorted((i, j))) for i, j in matcher.G.edges()}

        n_misses = len(edges_A ^ edges_B)

        return 1 - 2*n_misses/(n**2 - n)

    def component_compare(self, matcher):

        n = len(self.strings(min_count=0))

        components_A = [set(c) for c in self.components()]
        components_B = [set(c) for c in matcher.components()]
    ...
    def bp_compare(self, toCompare):
        ...
        g1_HF = self._compute_homophily_factor(g1D)
        g2_HF = self._compute_homophily_factor(g2D)

        (g1a, g1c_prime) = self._compute_FaBP_constants_from_homophily_factor(g1_HF)
        (g2a, g2c_prime) = self._compute_FaBP_constants_from_homophily_factor(g2_HF)
    ...
    def _compute_homophily_factor(self, D):
    ...
    def _compute_FaBP_constants_from_homophily_factor(self, HF):
    ...
    def plotMatches(self, saveFile=True, string=None, ax=None, cmap='tab10'):
        G = self.matches(string)
